from ironic_python_agent import utils, errors
from oslo_concurrency import processutils

from cern_ironic_hardware_manager import LOG


def upgrade_example_device_model1234_firmware(node, ports):
...
def check_ipmi_users(node, ports):
...
def validate_configuration(raid_config):
...
def create_configuration(node, ports):
    ...
    if not validate_configuration(raid_config):
...
def delete_configuration(node, ports):
